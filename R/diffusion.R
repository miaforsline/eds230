...
#' @param dx length of each segment/box/space (m)
...
diff1 = function(initialC, nx, dx, nt, dt,  D, area) {
  ...
  # create a data structure to store concentrations, at nx points and nt times
	...
	# also keep track of fluxes in an out of each point at each time (how much pollutant is moving in vs out of each box?)
...
#for each time step, how much is coming in/out, then calculate the concentration

	return(list(conc=conc,qout=qout,qin=qin)) #return arrays for concentration, input, and output 
}
